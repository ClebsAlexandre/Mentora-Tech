  ...
  return (
    <div className="flex flex-col min-h-screen bg-zinc-50 dark:bg-[#0a0a0a] font-sans text-zinc-900 dark:text-zinc-50">
      ...
      <main className="flex-1 flex flex-col items-center w-full">
        ...
        <section id="como-funciona" className="w-full bg-zinc-100 dark:bg-zinc-900/50 py-24 border-y border-zinc-200 dark:border-zinc-800/50">
          <div className="max-w-7xl mx-auto px-6">
            <div className="text-center mb-16">
              <h2 className="text-3xl font-bold tracking-tight mb-4">Como Funciona?</h2>
              <p className="text-zinc-600 dark:text-zinc-400 max-w-2xl mx-auto">Em apenas três passos você estará conectado com profissionais que podem transformar a sua jornada no desenvolvimento de software.</p>
            </div>
        ...
        {/* Benefícios Section */}
        <section id="beneficios" className="w-full py-24 max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold tracking-tight mb-4">O que você vai aprender</h2>
            <p className="text-zinc-600 dark:text-zinc-400 max-w-2xl mx-auto">Uma mentoria bem direcionada economiza meses de estudo solitário. Focamos nos pilares reais que o mercado exige.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: "💻", title: "Code Review", desc: "Tenha seu código JavaScript, React ou Node.js revisado por olhos experientes." },
              { icon: "🎯", title: "Entrevistas", desc: "Simule entrevistas técnicas para vagas de Júnior ou Estágio e receba feedback imediato." },
              { icon: "🏗️", title: "Arquitetura", desc: "Aprenda a estruturar bancos de dados em PostgreSQL e escalar aplicações com AWS e Docker." },
              { icon: "🚀", title: "Plano de Carreira", desc: "Descubra o próximo passo certo, seja na sua residência atual ou em novas vagas no mercado." }
            ].map((ben, i) => (
              <div key={i} className="p-6 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-blue-500/50 transition-colors text-left">
                <div className="text-3xl mb-4">{ben.icon}</div>
                <h3 className="font-bold text-lg mb-2">{ben.title}</h3>
                <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">{ben.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="mentores" className="w-full bg-zinc-100 dark:bg-zinc-900/30 py-24 border-t border-zinc-200 dark:border-zinc-800/50">
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {mentores.map((mentor) => (
                <div key={mentor.id} className="flex flex-col bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all hover:-translate-y-1 text-left group">
                  <div className="p-6 flex flex-col items-center text-center gap-4 flex-1">
                    {mentor.avatarUrl && (
                      <img src={mentor.avatarUrl} alt={`Foto de ${mentor.nome}`} className="w-24 h-24 rounded-full border-4 border-zinc-50 dark:border-zinc-800 shadow-sm object-cover group-hover:border-blue-500/30 transition-colors" />
                    )}
                    <div>
            ...
            {mentores.length === 0 && (
                <div className="text-center bg-white dark:bg-zinc-900 border border-dashed border-zinc-300 dark:border-zinc-700 rounded-2xl py-12">
                  <p className="text-zinc-500">Nenhum mentor encontrado. Rode a rota de seed ou cadastre um novo!</p>
                </div>
            )}
          </div>
        </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            ...
            <div className="col-span-1 md:col-span-1 lg:col-span-2">
              ...
              <p className="text-zinc-600 dark:text-zinc-400 text-sm max-w-sm leading-relaxed mb-6">
                A ponte entre o seu talento e as melhores oportunidades do mercado de tecnologia. Conecte-se, aprenda e evolua com quem já chegou lá.
              </p>
              
              {/* Ícones de Redes Sociais */}
              <div className="flex items-center gap-4 text-zinc-400">
                <a href="#" className="hover:text-blue-600 dark:hover:text-blue-500 transition-colors" aria-label="GitHub">
                ...
                <a href="#" className="hover:text-blue-600 dark:hover:text-blue-500 transition-colors" aria-label="LinkedIn">
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path fillRule="evenodd" d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" clipRule="evenodd" /></svg>
                </a>
                <a href="#" className="hover:text-blue-600 dark:hover:text-blue-500 transition-colors" aria-label="Twitter">
            ...
            {/* Coluna 2: Plataforma */}
            <div>
              <h3 className="font-semibold text-zinc-900 dark:text-zinc-100 mb-4">Plataforma</h3>
              <ul className="flex flex-col gap-3 text-sm text-zinc-600 dark:text-zinc-400">
                <li><Link href="#mentores" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Encontrar Mentores</Link></li>
                <li><Link href="#como-funciona" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Como Funciona</Link></li>
                <li><Link href="/cadastro" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Quero ser Mentor</Link></li>
                <li><Link href="#beneficios" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Benefícios</Link></li>
              </ul>
            </div>

            {/* Coluna 3: Suporte & Legal */}
            <div>
              <h3 className="font-semibold text-zinc-900 dark:text-zinc-100 mb-4">Suporte</h3>
              <ul className="flex flex-col gap-3 text-sm text-zinc-600 dark:text-zinc-400">
                <li><Link href="#" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Central de Ajuda</Link></li>
                <li><Link href="#" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Termos de Uso</Link></li>
                <li><Link href="#" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Privacidade</Link></li>
                <li><Link href="#" className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Contato</Link></li>
              </ul>
            </div>
          </div>
          ...
          <div className="pt-8 border-t border-zinc-200 dark:border-zinc-800/50 flex flex-col md:flex-row items-center justify-between gap-4">
            <p className="text-sm text-zinc-500 dark:text-zinc-500">
              © {new Date().getFullYear()} Mentora Tech. Todos os direitos reservados.
            </p>
